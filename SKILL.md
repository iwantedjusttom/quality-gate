---
name: quality-gate
description: The independent proof-of-correctness gate between "built" and "shipped" — the station that makes build-loop ship solid, secure software instead of just moving cards. Use it whenever a feature/fix needs to be PROVEN before it advances: when build-loop finishes an issue and is about to move it to In-Review, or when Tom says "is this actually tested", "prove it works", "run the quality gate", "verify this branch/PR", "is this secure", "gate this before I merge", or wants confidence in a change before it ships. It spawns a FRESH agent (never the builder) to prove a change against its spec — writing tests, running the app to observe real behavior, ticking each acceptance box only once proven, plus adversarial `code-review` and a two-lens security pass (`security-review` + `vibe-security`) — then returns a PASS/FAIL verdict with a short report. On FAIL it files the findings back as fixes; only a real PASS — every acceptance box ticked — lets a card reach In-Review. Companion to build-loop (which calls it) and design-queue (whose issue body is the spec it tests against); it never designs and never decides scope — it only proves.
---
